...
func register(s *mcp.Server, q *query.Engine) {
	...
	}, func(ctx context.Context, req *mcp.CallToolRequest, in searchIn) (*mcp.CallToolResult, []query.SymbolHit, error) {
		hits, err := q.SearchSymbols(in.Query, in.Kind, limit(in.Limit))
		return nil, nonNil(hits), err
	})
	...
	}, func(ctx context.Context, req *mcp.CallToolRequest, in getSymbolIn) (*mcp.CallToolResult, []symbolOut, error) {
		hits, err := q.GetSymbol(in.FQN)
		if err != nil {
			return nil, nil, err
		}
		if len(hits) == 0 {
			return nil, nil, fmt.Errorf("symbol %q not found; try search_symbols", in.FQN)
		}
	...
	}, func(ctx context.Context, req *mcp.CallToolRequest, in fqnIn) (*mcp.CallToolResult, []query.EdgeHit, error) {
	...
	type hierarchyOut struct {
		Ancestors   []query.Relative `json:"ancestors,omitempty"`
		Descendants []query.Relative `json:"descendants,omitempty"`
	}
	mcp.AddTool(s, &mcp.Tool{
	...
	}, func(ctx context.Context, req *mcp.CallToolRequest, in outlineIn) (*mcp.CallToolResult, []query.SymbolHit, error) {
		hits, err := q.FileOutline(in.Path)
		if err == nil && len(hits) == 0 {
			return nil, nil, fmt.Errorf("no symbols for %q — check that the path is project-root-relative", in.Path)
		}
		return nil, hits, err
	})
}
